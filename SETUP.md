...
**Verified 2026-08-23** (see `deployments.md`, "Unlisted-upload detection assumption (F4)" for the full probe). Step 1 below depends on the channel's uploads playlist (`playlistItems.list`, the mechanism `listRecentVideos` uses) returning an unlisted video once the request is OAuth-authenticated as the channel owner. Confirmed empirically with two throwaway unlisted clips: both appeared in the uploads playlist under the OAuth client with `status.privacyStatus=unlisted` within ~15 seconds of upload (0 items at t+8s — propagation lag; 1 item at t+16s, while the video's own `processingStatus` was still `processing`). The API-key client never saw either video (404 `playlistNotFound`, as expected — the public listing only ever shows public uploads). The 10-minute F4 poll interval has ample margin over that ~15s propagation window; no code change needed here. Both probe videos were deleted afterward and the channel is back to 0 uploads.
